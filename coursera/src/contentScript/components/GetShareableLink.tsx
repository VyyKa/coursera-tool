...
export default function GetShareableLink() {
  ...
  return (
    <div className="flex items-center mt-3 mb-3 text-sm">
      <span
        className="flex-shrink-0 z-10 inline-flex items-center py-2 px-3 font-medium text-center text-white border border-green-500 rounded-s-lg hover:bg-green-600 hover:border-green-600 bg-green-500 cursor-pointer"
        onClick={async () => {
          setIsLoading(true);
      ...
      <div className="relative w-full">
        ...
        className="bg-gray-50 border border-e-0 !text-black border-zinc-300 border-s-0 focus:ring-green-500 focus:border-green-500 block w-full p-2"
          value={url}
          readOnly
          disabled
        />
      ...
      <button
        ...
        className="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-white bg-green-600 rounded-e-lg hover:bg-green-700 focus:ring-0 focus:outline-none border border-green-600 hover:border-green-700"
        type="button"
        onClick={handleCopy}
      >
        {!isCopied ? (
